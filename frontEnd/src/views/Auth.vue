...
const signUp = async () => {
  try {
    const response = await axios.post('http://localhost:8000/api/user/createUser', {
...
const signIn = async() => {
  // console.log('Sign In:', {  //   email: signInEmail.value,
  //   password: signInPassword.value
  // });
  try {
    const response = await axios.post('http://localhost:8000/api/user/loginUser', {
      email: signInEmail.value,
      password: signInPassword.value
    });
    if(response.data.rows[0]){
      console.log('Login Success:', response.data.rows[0]);
      emit('update-auth-user',true);
      
    }else{
      console.log("login yoki parol hato");
